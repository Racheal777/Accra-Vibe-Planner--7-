...
export const useVibePlanner = ({ setPlanHistory }: UseVibePlannerProps) => {
  ...
  const handleSubmit = async (finalParams: HangoutParams) => {
    trackEvent('plan_generated', {
      mode: finalParams.planningMode || 'detailed',
      vibe: finalParams.vibe || 'unknown',
      budget: finalParams.budget || 'unknown',
      timing: finalParams.timing || 'unknown',
    });
    setAppState('LOADING');
    try {
  ...
  const handleOptionSelect = (key: keyof HangoutParams, value: any) => {
    trackEvent('question_answered', { key, value: String(value) });
  ...
  const handleStartPlanning = (planningMode: 'quick' | 'detailed') => {
    trackEvent('plan_started', { mode: planningMode });
  ...
  const handlePlanSelect = (plan: string) => {
    trackEvent('card_selected', { title: plan.split('\n')[1] || 'unknown' });
  ...
  const handleLocationSubmit = async (origin: string, originCoords: Location | null, intendedTime: string) => {
    // Deprecated. Transitioning directly via handlePlanSelect instead.
  };
