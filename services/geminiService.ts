...
import { AI } from '../config/appConfig';
...
export async function generatePlanOptions(params: HangoutParams): Promise<string> {
  ...
  const vibeExplanation: Record<Vibe, string> = {
    ...
    'Sports & Games': "The user wants to engage in sophisticated or less common sports. Suggest places for activities like tennis, badminton, pilates, swimming, golf, horse riding, or yoga. Look for sports clubs, specialized studios, or high-end recreational facilities.",
  ...
  const prompt = `
      ...
      2. The Vibe Category for the plan, which MUST be one of the following: "Relax & Unwind", "Food & Nightlife", "Sports & Games", "Active & Adventure", "Movies & Plays", "Romantic Date", "Picnic & Parks".
      3. A specific, searchable location for the venue. This must be a full address or a well-known landmark name that can be accurately found on Google Maps (e.g., "Republic Bar & Grill, Osu, Accra" or "Kwame Nkrumah Memorial Park & Mausoleum"). Do NOT just provide a general neighborhood.
      4. The Google Maps rating (e.g., "Rating: 4.5/5 stars"). If unavailable, state "Rating: Not available".
      5. The opening hours for the venue (e.g., "9:00 AM - 10:00 PM"). If unavailable, state "Not available".
      6. An "Essentials Checklist" with the following details, each on a new line starting with a hyphen:
      ...
      7. A fun, detailed, and realistic description of the activities. The description must be one concise and exciting sentence.
      8. An estimated per-person cost in Ghanaian Cedis (GH₵).
      9. "Estimated Ride Cost:" An estimate of Bolt/Uber rideshare cost from the user's location (if provided) to the venue in GH₵. Give a range (e.g., GH₵30 - GH₵50).
      10. "Weather:" The expected weather around the specific time. Be concise.
      11. A short, useful pro-tip.
      ${picnicInstructions}
      ...
      Estimated Ride Cost: [Ride cost estimate for plan 1]
      Weather: [Weather forecast for plan 1]
      ...
      Estimated Ride Cost: [Ride cost estimate for plan 2]
      Weather: [Weather forecast for plan 2]
  ...
  try {
    ...
    const cleanedText = initialText.replace(/\[\d+(?:\s+in\s*(?:search|search\s+\d+))?\]/g, '');
    return cleanedText;
  } catch (error) {
